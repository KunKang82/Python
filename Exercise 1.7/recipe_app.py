...
def create_recipe():
    ...
    # Call calculate_difficulty() that calculates the difficulty of the recipe by taking in cooking_time and the number of ingredients
    difficulty = calculate_difficulty(cooking_time, len(recipe_ingredients))
...
def edit_recipe():
    ...
    if session.query(Recipe).count() == 0:
        ...
        # Print the list of available recipes with IDs
        ...
        recipes_id_list = [recipe[0] for recipe in results]
        ...
        if column_for_update == 1: 
            ...
            updated_value = input("Enter the new value for the ingredients (each separated by a comma followed by a space): ")
            # Split the input into a list of ingredients
            updated_ingredients = [ingredient.strip() for ingredient in updated_value.split(',')]
            # Join the ingredients into a string before updating the database
        ...
        updated_difficulty = calculate_difficulty(recipe_to_edit.cooking_time, len(recipe_to_edit.ingredients.split(", ")))
